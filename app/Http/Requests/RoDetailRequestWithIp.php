...
use App\Rules\ValidIpsForRo;
use App\Rules\ValidProductsForRo;

class RoDetailRequestWithIp extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $certificateNoExistRule = 'exists:applications,certificate_no,app_type_id,' . config('app_constants.app_types.IP');
        ...
        $certificateNoExistRule .= ',applicant_id,' . $applicantId;
        ...
        if($applicationId > 0) {
            ...
            $rules1 = [
                'ro_ip_application' => ['required', 'array', 'min:1', new ValidIpsForRo(request()->all(), $applicationId, $applicantId)],
                'ro_ip_application.*.certificate_no' => "required|$certificateNoExistRule"
            ];
        }
        ...
        $rules3 = [
            ...
            'attachments.extension_attachment' => isset($inputs['is_extension_letter_required'])
                && $inputs['is_extension_letter_required'] === true ?
            ...
            'app_products' => ['required', 'array', 'min:1', new EmptyAppProduct, new ValidProductsForRo(request()->all(), $applicationId, $applicantId)],
            ...
            'applications.cnf_ip_assignment_id' => 'nullable|numeric'
        ];
